...
export function NotificationBell({ count = 0, onClick }: NotificationBellProps) {
  return (
    ...
    >
      ...
      {count > 0 && (
        <span className="absolute top-0 right-0 flex items-center justify-center min-w-[18px] h-[18px] text-xs font-bold bg-[var(--color-primary)] text-white rounded-full px-1">
          {count}
        </span>
      )}
    </button>
